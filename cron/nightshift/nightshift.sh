...
#       Run one Pi parent session that orchestrates all dirty review feature lanes
#       with subagents and updates the dynamic clean-state ledger in review.md.
#
#   nightshift.sh <slot-index> [timeout_min]
#       Deprecated legacy/manual compatibility mode: run one static phase (slot 0-8)
#       only when review.md still has ## Phase N headings. Current feature-lane
#       review docs normally do not; launchd should not use this mode anymore.
#
...
#
# Feature lanes and the dynamic clean-state ledger are read from review.md (same
# directory as this script). The orchestrator mode treats review.md as mutable
# state: clean feature lanes should update their ledger rows so future runs skip
# unchanged code.
...
# ── Build prompt ─────────────────────────────────────────────
if [[ "$MODE" == "orchestrate" ]]; then
  TASK_ONELINE="Single Pi parent orchestration using review.md feature ledger"
...
Your task: run the entire Night Shift as one parent session, using Pi subagents for dirty feature-lane triage and fixes instead of running independent top-level sessions.
...
- Read review.md's Dynamic Clean-State Ledger first. Use each row's Feature, Path filter, Clean through commit, and Reopen trigger to decide which feature lanes reopened.
- If a feature lane's Path filter did not change since its Clean through commit and its cheapest relevant validation has no High Finding or failure, skip deep review for that feature lane.
- If a feature lane is pending, dirty, or has failing validation, triage that feature lane only. Do not browse unrelated clean directories just in case.
- Respect deferred feature lanes in review.md. Do not audit TockDesigner or Calls unless review.md or the user explicitly reactivates them.
...
- If a feature lane has no High Findings after triage, update its review.md ledger row with the clean commit/date/evidence so future agents skip unchanged clean code.
...
- At the end, summarize feature lanes skipped, feature lanes triaged, fixes committed, ledger rows updated, validations run, and any residual risks."
else
...
- If a feature lane has no High Findings, update its review.md ledger row so future agents skip unchanged clean code.
