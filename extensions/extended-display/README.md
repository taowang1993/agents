...
- Before target app code loads, CJS and ESM Electron imports receive guarded BrowserWindow/BaseWindow constructors. Windows are constructed hidden and nonfocusable on a non-main, non-overlapping display. Top-level verification windows fill its entire work area before visibility and stay that size, even if app code requests smaller bounds; owned child dialogs retain their requested size within that work area. This is display-filling window geometry, not macOS full-screen Spaces. Bounds are checked before `showInactive()`. Later moves/resizes are constrained; main-display IDs and mirror-only layouts are rejected. Display removal/primary reassignment hides and destroys owned windows and terminates the app.
...
If the application needs a standard custom protocol before Electron is ready, place a JSON array of names (for example `["app"]`) at `<entry>.protocols.json`. The bootstrap registers these before readiness without running any target code early. At most eight unique custom names are accepted; built-in schemes and arbitrary privilege overrides are rejected. Registered schemes use fixed standard/secure/fetch/stream/code-cache/CORS support, never CSP bypass. Target app loading still follows display validation and window-guard installation.
