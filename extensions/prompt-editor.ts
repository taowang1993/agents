...
const PREFERRED_MODE_NAMES = ["gpt-5.6-sol", "kimi-k3", "deepseek-v4-pro"] as const;
...
function isPreferredModeName(name: string | null | undefined): name is PreferredModeName {
	return typeof name === "string" && (PREFERRED_MODE_NAMES as readonly string[]).includes(name);
}

function getPreferredToggleTarget(current: string | undefined, available: PreferredModeName[]): PreferredModeName | undefined {
	if (available.length === 0) return undefined;

	// Find the current mode in the available list and cycle to the next one.
	const idx = typeof current === "string" ? available.indexOf(current as PreferredModeName) : -1;
	if (idx >= 0 && idx < available.length - 1) return available[idx + 1];

	// Current is last (or not found) — wrap to the first.
	return available[0];
}

async function togglePreferredMode(pi: ExtensionAPI, ctx: ExtensionContext): Promise<void> {
	await ensureRuntime(pi, ctx);

	const available = PREFERRED_MODE_NAMES.filter((name) => runtime.data.modes[name]) as PreferredModeName[];
	if (available.length === 0) {
		if (ctx.hasUI) {
			ctx.ui.notify(`Missing preferred modes in modes.json: ${PREFERRED_MODE_NAMES.join(", ")}`, "warning");
		}
		return;
	}

	const inferred = inferModeFromSelection(ctx, pi, runtime.data);
	const current = isPreferredModeName(runtime.currentMode)
		? runtime.currentMode
		: isPreferredModeName(inferred)
			? inferred
			: undefined;

	const next = getPreferredToggleTarget(current, available);
	if (!next) {
		if (ctx.hasUI) ctx.ui.notify("No preferred mode available", "warning");
		return;
	}
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerShortcut("shift+\\", {
		description: "Cycle through gpt-5.6-sol, deepseek-v4-pro",
		handler: async (ctx) => {
			await togglePreferredMode(pi, ctx);
		},
	});
