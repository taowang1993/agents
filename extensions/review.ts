 ...
 * - If a .context/review.md file exists in the same directory as .pi,
...
async function loadProjectReviewGuidelines(cwd: string): Promise<string | null> {
	...
	while (true) {
		...
		const guidelinesPath = path.join(currentDir, ".context", "review.md");
...
export default function reviewExtension(pi: ExtensionAPI) {
	 ...
	 */
	async function getSmartDefault(): Promise<"uncommitted" | "baseBranch" | "commit" | "custom"> {
		return "custom";
	}
		...
		handler: async (args, ctx) => {
			...
			while (true) {
				...
				if (messageCount > 0) {
					...
					const choice = await ctx.ui.select("Start review in:", ["Current Session", "Empty Branch"]);
