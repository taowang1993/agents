...
- Hostname observed over SSH: `MacBook-Pro.lan`
- Current IP: unknown; do not reuse the MacBook Air IP
...
If the user says only “the other laptop” without identifying Air or Pro, ask which MacBook to use before running commands.
...
Use `BatchMode=yes` so an unavailable key or host fails promptly instead of waiting for a password. The current SSH key is authorized for both known accounts.
...
If the MacBook Air hostname fails, retry `tao@192.168.1.71`. If the MacBook Pro hostname fails, ask the user for its current IP rather than guessing or using the Air's IP.
