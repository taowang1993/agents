...
Verify mcporter can see Exa:
```bash
npx mcporter list exa
```

If not listed:
```bash
npx mcporter config add exa https://mcp.exa.ai/mcp
```
...
```bash
npx mcporter call 'exa.web_search_exa(query: "latest AI news 2026", numResults: 5)'
```
...
```bash
npx mcporter call 'exa.get_code_context_exa(query: "React hooks examples", tokensNum: 3000)'
```
...
```bash
npx mcporter call 'exa.company_research_exa(companyName: "Anthropic", numResults: 3)'
```
...
**Enable all tools:**
```bash
npx mcporter config add exa-full "https://mcp.exa.ai/mcp?tools=web_search_exa,web_search_advanced_exa,get_code_context_exa,deep_search_exa,crawling_exa,company_research_exa,people_search_exa,deep_researcher_start,deep_researcher_check"

# Then use:
npx mcporter call 'exa-full.deep_search_exa(query: "AI safety research")'
```
