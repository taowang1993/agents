...
| `~/.bun/install/cache`, `~/.npm/_npx`, `~/.npm/_cacache`, `~/.cache/uv`, `~/Library/Caches/pnpm`, `~/Library/Caches/ms-playwright` | Package manager and browser-test caches/temp installs. Removed about 5.3GB on 2026-06-26; removed `~/.npm/_npx` 1.4GB and `~/.cache/uv` 10.7MiB on 2026-07-07. | Prefer tool cleanup where available: `/Users/max/.nvm/versions/node/v22.22.0/bin/npm cache clean --force`, `uv cache clean`; then delete only cache/temp dirs such as Bun install cache, `~/.npm/_npx`, pnpm cache, and Playwright browser cache. Do **not** confuse `~/Library/Caches/pnpm` with `~/Library/pnpm/store`. |
