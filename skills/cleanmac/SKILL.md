...
| `~/.gradle/{wrapper/dists,daemon,caches}` | Gradle wrapper downloads, daemons, and caches used by Android/Expo local builds. Removed stale wrapper dists (`gradle-8.8-all`, `gradle-8.9-bin`, `gradle-8.13-bin`) plus daemon logs, shrinking `~/.gradle` from 940MB to 147MB on 2026-07-07. | Expo Android builds may need Gradle, so do not delete the whole `~/.gradle` directory unless the user accepts redownloading. Confirm no Gradle/Java build is running, then delete stale wrapper dists or caches; projects redownload the requested Gradle version. |
