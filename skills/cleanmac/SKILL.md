...
| `~/.cache/codex-runtimes` | Codex CLI/runtime cache. Removed 1.5GB on 2026-07-07. | Confirm `lsof +D ~/.cache/codex-runtimes` is empty, then delete the cache directory; Codex recreates or redownloads it. |
| `~/.cache/*` | XDG-style per-user CLI/tool caches such as opencode, corepack, giget, GitHub CLI, and MCP caches. Removed the remaining 473MB on 2026-07-07 after confirming no open files. | Confirm `lsof +D ~/.cache` is empty, then delete children of `~/.cache`, not the parent; tools recreate caches and may redownload package-manager shims. |
| `/private/var/folders/.../X/com.microsoft.edgemac.code_sign_clone` | Microsoft Edge code-signing/app bundle clones. Removed 8.9GB on 2026-06-21. Found 6 active clones totaling 5.3GB on 2026-07-07. Removed 8.9GB on 2026-07-08 after Edge was uninstalled. | Quit Edge with AppleScript, confirm `lsof +D` is empty, then `rm -rf` only this folder. |
| `/private/var/folders/.../X/com.brave.Browser.code_sign_clone` | Brave Browser code-signing/app bundle clones. Removed empty leftover on 2026-07-08 after Brave was uninstalled. | Quit Brave with AppleScript, confirm `lsof +D` is empty, then `rm -rf` only this folder. |
...
| `~/Library/Application Support/Open Design` | Open Design app support, launcher/runtime channels, and namespaces. Removed 1.4GB on 2026-07-07 after user confirmed removal. | Delete only when Open Design is not running and the user no longer needs its local state/runtime. |
...
| `/private/var/folders/.../T/_MEI*` | PyInstaller one-file extraction directories, often containing temporary Python/MLX runtime bundles. Observed `_MEI0eituj` at 1.3GB on 2026-08-03. | Confirm `lsof +D` is empty and no process references the directory, then delete only the stale `_MEI*` directory; the executable recreates it when needed. |
| `/private/var/folders/.../T/{tockslider-electron-e2e-*,tockcoder-electron-e2e-*,tockspeaker-whisper-*,metro-cache,jest_dx,esbuild-*,pi-clipboard-*.png,pi-bash-*.log}` | Test bundles, Metro/Jest/esbuild caches, Pi clipboard images, and Pi bash logs in the user temp directory. Removed about 2GB on 2026-07-07. | Confirm no matching test/dev process is running, then delete only these temp patterns. |
...
| `~/Library/pnpm/store` | pnpm content-addressable package store. Store format dirs are not package versions. | Default to **skip `pnpm store prune`** on Max's machine. It removed the pinned pnpm 11.1.2 package-manager path under `~/Library/pnpm/store/v11/links/@/pnpm/...` on 2026-06-26, which broke the npm shim/Codex update flow. Remove old store dirs like `v3`/`v10` only when the active store is another dir. Removed `v3` 2.3GB and `v10` 10GB on 2026-06-21; removed stale `v10` 1.6GB on 2026-07-07 while active store was `v11`; `v11` pruned 2.8GB on 2026-06-26; removed `/Users/max/Library/pnpm/store/v10` (2.5GB) on 2026-08-03 after verifying `v11` was active. If the user explicitly wants prune, warn first, then verify afterward from a pnpm-pinned project: `pnpm -v`, `/Users/max/Library/pnpm/pnpm -v`, and `npm -v`. |
| `~/.bun/install/cache`, `~/.npm/_npx`, `~/.npm/_cacache`, `~/.cache/uv`, `~/Library/Caches/{CocoaPods,electron,@multicadesktop-updater,go-build,copilot,tockspeaker,node-gyp,pnpm,Homebrew,ms-playwright}` | Package manager, build, updater, and browser-test caches/temp installs. Removed about 5.3GB on 2026-06-26; removed `~/.npm/_npx` 1.4GB and `~/.cache/uv` 10.7MiB on 2026-07-07; removed another 2.3GB of `~/Library/Caches` children on 2026-07-07. | Prefer tool cleanup where available: `/Users/max/.nvm/versions/node/v22.22.0/bin/npm cache clean --force`, `uv cache clean`; then delete only cache/temp dirs such as Bun install cache, `~/.npm/_npx`, pnpm cache, and Playwright browser cache. Do **not** confuse `~/Library/Caches/pnpm` with `~/Library/pnpm/store`. |
| `~/Library/Caches/dotslash` | Dotslash app/tool cache. On 2026-07-07, `React Native DevTools.app` leftovers with `com.apple.macl` blocked `rm -rf`, leaving 258MB. | Try normal `rm -rf` first. If macOS denies deletion despite ownership, leave it or remove via Finder/Full Disk Access rather than fighting SIP/TCC from the agent. |
| `~/.bun/install/global/node_modules` | Bun global package install tree. Removed 1.2GB on 2026-07-07 after confirming no global Bun bins were present. | Remove only when the user accepts reinstalling Bun global packages; keep `~/.bun` itself. |
| `~/.expo/{ios-simulator-app-cache,expo-go,*-cache}` | Expo simulator app, Expo Go, template, schema, and native module caches. Removed 706MB on 2026-07-07. | Safe for Expo users; Expo recreates/redownloads these caches. Do not delete auth/state files unless intentionally resetting Expo. |
| `~/.codex/.tmp` | Codex temporary plugin and marketplace sync cache. Removed 303MB on 2026-07-07. | Confirm no Codex plugin sync is active; then delete `.tmp`. Codex recreates it. |
...
| `~/.gradle/{wrapper/dists,daemon,caches}` | Gradle wrapper downloads, daemons, and caches used by Android/Expo local builds. Removed stale wrapper dists (`gradle-8.8-all`, `gradle-8.9-bin`, `gradle-8.13-bin`) plus daemon logs, shrinking `~/.gradle` from 940MB to 147MB on 2026-07-07; removed `~/.gradle/caches` (6.1GB, including the 4.4GB Gradle 9.3.1 cache) on 2026-08-03 after confirming no Gradle process or open handles. | Expo Android builds may need Gradle, so do not delete the whole `~/.gradle` directory unless the user accepts redownloading. Confirm no Gradle/Java build is running, then delete stale wrapper dists or caches; projects redownload the requested Gradle version. |
