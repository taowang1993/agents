...
# Check Corepack before pruning pnpm — prune removes Corepack-managed pnpm versions
corepack --version 2>/dev/null && echo "⚠️  Corepack active — pnpm store prune will nuke Corepack-downloaded pnpm versions" || true
...
| `~/Library/pnpm/store` | pnpm content-addressable package store. Store format dirs are not package versions. | Run `pnpm store path` and `pnpm store prune`; remove old store dirs like `v3`/`v10` only when the active store is another dir. Removed `v3` 2.3GB and `v10` 10GB on 2026-06-21; `v11` pruned 2.8GB on 2026-06-26. **⚠️ Corepack trap:** if Corepack manages pnpm (check `which pnpm` resolves to a corepack shim under `~/.nvm`), `pnpm store prune` will delete Corepack-downloaded pnpm versions from the store. Projects pinned to a different version than the global one will break until Corepack re-downloads. Check `corepack --version` first; if active, warn the user and skip prune unless they're ok with a re-download on next `pnpm install`. |
